...
logging.basicConfig(
    ...
    handlers=[
        #logging.FileHandler(f'LowSciBertTraining_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
class SiameseNetwork(nn.Module):
    def __init__(self, scibert_size=768):
        ...
        self.paper_encoder = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(0.15),
            nn.Linear(512, 384),  # Reduced from 512
            nn.LayerNorm(384),
            nn.GELU(),
            nn.Dropout(0.15)
        )
        ...
        self.metadata_encoder = nn.Sequential(
            ...
            nn.LayerNorm(64),
            nn.GELU(),
            nn.Linear(64, 128),  # Increased from 64
            nn.LayerNorm(128)
        )
    ...
    def forward(self, paper1, paper2, metadata):
        ...
        weights = F.softmax(torch.stack([self.scibert_weight, self.metadata_weight]), dim=0)
        p1_encoded = p1_encoded * weights[0]
        p2_encoded = p2_encoded * weights[0]
        meta_encoded = meta_encoded * weights[1]
...
def setup_folders():
    ...
    base_dir = f"LowSciBertexperiments/run_{timestamp}"
